...
abstract class RIAutomobilityLangProvider extends LanguageProvider {
    ...
    protected final void addVehicleImportTranslations(boolean chinese) {
        ...
        add(p + "label.hide_engine.off", chinese ? "否" : "No"); add(p + "label.hide_engine.on", chinese ? "是" : "Yes");
        add(p + "label.axle.rear", chinese ? "后" : "Rear"); add(p + "label.axle.front", chinese ? "前" : "Front");
        add(p + "label.side.right", chinese ? "右" : "Right"); add(p + "label.side.left", chinese ? "左" : "Left");
        add(p + "label.engine_animation.stopped", chinese ? "停止" : "Stopped"); add(p + "label.engine_animation.running", chinese ? "运行" : "Running");
        add(p + "label.container_hitbox.off", chinese ? "否" : "No"); add(p + "label.container_hitbox.on", chinese ? "是" : "Yes");
        add(p + "label.front_attachment", chinese ? "前附件：" : "Front Attachment:"); add(p + "label.rear_attachment", chinese ? "后附件：" : "Rear Attachment:");
        add(p + "label.front_list", chinese ? "前名单：" : "Front List:"); add(p + "label.rear_list", chinese ? "后名单：" : "Rear List:");
        add(p + "label.front_attachment_position", chinese ? "前附件位：" : "Front Attachment Position:"); add(p + "label.rear_attachment_position", chinese ? "后附件位：" : "Rear Attachment Position:");
        add(p + "label.entity_width", chinese ? "实体宽度：" : "Entity Width:"); add(p + "label.entity_height", chinese ? "实体高度：" : "Entity Height:");
        add(p + "label.preview_view", chinese ? "预览视角：" : "Preview View:");
        add(p + "label.front_attachment.disabled", chinese ? "禁用" : "Disabled"); add(p + "label.front_attachment.enabled", chinese ? "启用" : "Enabled");
        add(p + "label.rear_attachment.disabled", chinese ? "禁用" : "Disabled"); add(p + "label.rear_attachment.enabled", chinese ? "启用" : "Enabled");
        add(p + "label.front_list.blacklist", chinese ? "黑名单" : "Blacklist"); add(p + "label.front_list.whitelist", chinese ? "白名单" : "Whitelist");
        add(p + "label.rear_list.blacklist", chinese ? "黑名单" : "Blacklist"); add(p + "label.rear_list.whitelist", chinese ? "白名单" : "Whitelist");
        add(p + "option.first_person", chinese ? "第一人称" : "First Person"); add(p + "option.external", chinese ? "外部" : "External");
        ...
        add(p + "status.installing_component", chinese ? "正在生成并安装组件..." : "Generating and installing component..."); add(p + "status.item_export_failed", chinese ? "物品导出失败：%s" : "Item export failed: %s");
        add(p + "status.component_installed_syncing", chinese ? "组件已安装，正在同步客户端资源..." : "Component installed; synchronizing client resources..."); add(p + "status.item_exported", chinese ? "组件已安装，物品已发送到输出槽" : "Component installed; item sent to output slot");
        ...
        add(p + "tooltip.weight", chinese ? "车架的物理重量，影响加速、操控与抓地。" : "The frame's physical weight, affecting acceleration, handling, and grip.");
        add(p + "tooltip.engine_back", chinese ? "引擎相对车架中心向车尾移动的距离，单位为模型像素。" : "Engine offset toward the rear from the frame center, in model pixels.");
        add(p + "tooltip.engine_height", chinese ? "引擎相对车架的垂直位置，单位为模型像素。" : "Engine vertical offset from the frame, in model pixels.");
        add(p + "tooltip.forward", chinese ? "轮位沿车架前后方向的位置，单位为模型像素。" : "Wheel position along the frame's forward axis, in model pixels.");
        add(p + "tooltip.right", chinese ? "轮位沿车架左右方向的位置，单位为模型像素。" : "Wheel position along the frame's lateral axis, in model pixels.");
        ...
        add(p + "tooltip.grip", chinese ? "轮胎抓地参数，与车架重量共同决定整车抓地能力。" : "Tire grip parameter; together with frame weight it determines vehicle grip.");
        ...
        add(p + "tooltip.rotation_y.wheel", chinese ? "整个车轮模型的 Y 轴朝向修正，单位为度。" : "Whole wheel-model Y-axis orientation adjustment, in degrees.");
        ...
        add(p + "tooltip.x.seat", chinese ? "座椅局部 X 偏移，单位为方块。" : "Seat local X offset, in blocks."); add(p + "tooltip.y.seat", chinese ? "座椅局部 Y 偏移，单位为方块。" : "Seat local Y offset, in blocks."); add(p + "tooltip.z.seat", chinese ? "座椅局部 Z 偏移，单位为方块。" : "Seat local Z offset, in blocks.");
        add(p + "tooltip.x.hitbox", chinese ? "附加碰撞箱底面中心的 X 偏移，单位为方块。" : "Additional hitbox base-center X offset, in blocks."); add(p + "tooltip.y.hitbox", chinese ? "附加碰撞箱底面中心的 Y 偏移，单位为方块。" : "Additional hitbox base-center Y offset, in blocks."); add(p + "tooltip.z.hitbox", chinese ? "附加碰撞箱底面中心的 Z 偏移，单位为方块。" : "Additional hitbox base-center Z offset, in blocks.");
        ...
        add(p + "tooltip.axle", chinese ? "标记当前轮位属于前轴或后轴。" : "Marks the current wheel position as front or rear axle."); add(p + "tooltip.side", chinese ? "标记当前轮位位于车辆左侧或右侧。" : "Marks the current wheel position as the vehicle's left or right side.");
        ...
        add(p + "tooltip.front_attachment", chinese ? "控制是否允许安装前附件。" : "Controls whether front attachments may be installed."); add(p + "tooltip.rear_attachment", chinese ? "控制是否允许安装后附件。" : "Controls whether rear attachments may be installed.");
